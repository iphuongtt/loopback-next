...
This is a guide to describe how to migrate LoopBack 3 built-in User/AccessToken
model based authentication.

As the pre-work of this guide, we created an application completely migrated
from
[`loopback-example-access-control`](https://github.com/strongloop/loopback-example-access-control).
It builds with a LoopBack 4 authentication system based on
`@loopback/authentication` and an authorization system based on
`@loopback/authorization`. Our migration guide will use it as the reference. A
...
You can take a few minutes on reading
[this page](https://loopback.io/doc/en/lb3/Introduction-to-User-model-authentication.html)
to recall the authentication system in LoopBack 3.

LoopBack 3 provides a built-in `User` model, which can be easily migrated to
LoopBack 4 by `lb4 import-lb3-models`. It also provides an `AccessToken` model
to store a logged in user's information. Its authentication system's mechanism
is:
...
strategies, not only the token based one. To get an equivalent authentication
system in your LoopBack 4 application, you can achieve that easily and fast by
migrating the LoopBack 3 `User` model and plugging in a prototype JWT
authentication component to perform the user login, and to verify a user's
identity by token.

Here is a diagram that illustrates the scenario in LoopBack 4, boxes with a
darker blue color emphasize its differences from LoopBack 3.

![LoopBack 4 authentication](../../imgs/tutorials/authentication-migration/lb4-authentication-user.png)

As you could tell from the diagram, the concept of `User` stays the same, the
differences of the core authentication module mostly come from:

- LoopBack 3 provides persistency for access token, whereas our prototype
...
- migrates model definitions for model `User`
- provides persistency for `User`
- creates a login controller
- mounts the JWT authentication component, which mainly includes:
...
model, a repository, a controller for `User` to get an equivalent LoopBack 3
...
_Migrating all functions for User need more effort, this section only create
login function as the minimum requirement._
...
back with its identity information encoded and includes it in the header of next
requests.
...
As you could see, the login function is pretty light, all the complex utilities
are implemented by services. This also decouples the responsibilities. We will
create the token and user service in the next section.
...
in story [to be created](). Now you can import it from the example as a
workaround.
